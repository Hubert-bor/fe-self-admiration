...
export default class ThreeBase {
  ...
  mouseHover() {
    ...
    this.container.addEventListener('pointermove', (event) => {
      ...
      this.mouse1.y =
        -((event.offsetY - this.container.offsetTop) / this.container.offsetHeight) * 2 + 1
      const vector = new THREE.Vector3(this.mouse1.x, this.mouse1.y, 1).unproject(this.camera)
  ...
  setModelCenter(object, viewControl) {
    ...
    const centroid = new THREE.Vector3().copy(objSize)
